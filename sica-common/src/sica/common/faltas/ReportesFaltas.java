...
public class ReportesFaltas {
   // Se define el metodo de obtener las faltas del usuario en periodo
    public static Task<ObservableList<FaltasUsuario>> getFaltasUsuariosPeriodo(
        ...
        return new Task<ObservableList<FaltasUsuario>>() {
            @Override protected ObservableList<FaltasUsuario> call() throws Exception {
                ...
                // Se obtienen las listas de las consultas
                    ...
                    while (actual.getTime().compareTo(hasta)<=0  ){
                        ...
                        if (debioasistir){
                            ...
                            if (reg==null){
                                ...
                                // se agrega la coleccion de datos
                                fu.addFalta(f);
                            }
                        }
...
// Metodo para Contabilizar las faltas de los usuaruo por departamento
    public static Task<ObservableList<FaltasUsuario>> getFaltasAsignaturasDepartamentoPeriodo(
        ...
        return new Task<ObservableList<FaltasUsuario>>() {
            @Override protected ObservableList<FaltasUsuario> call() throws Exception {
                ...
                String desdeTxt = Utils.formatDate(desde);
                String hastaTxt = Utils.formatDate(hasta);
// Se obtienen las listas dependiendo el tipo de usaurio y sus privilegios
                ObservableList<HorarioCrn> materias = (usuario.getPrivilegios() == Privilegios.USUARIO)?
                  ...
                  //     DBQueries.get_checking_regfull(desdeTxt, hastaTxt, depto.getCodigo());
                ...
                ObservableList<JustificantePeriodo> justif = DBQueries.getJustificantesAprobadosPeriodo(desdeTxt,hastaTxt);
           //   ObservableList<HorarioCrn> check = DBQueries.get_checking_regfull(desdeTxt, hastaTxt, depto.getCodigo());
            // Se declara el mapa que contendra las faltas 
                    ...
                    while(actual.getTime().compareTo(hasta) <= 0){
                        ...
                        if (actual.getTime().compareTo(h.getInicioDate()) >=0 && 
                            ...
                            if ( find == null){
                               ...
                               // falta.setChecado(h.getChecado1());

                                Crn crn = new Crn();
                                crn.setCrn(h.getCrn());
                                crn.setMateria(h.getMateria());
                                crn.setCodProf(h.getUsuario()); 
                                Registro checados = new Registro();
                                checados.setFechahora(h.getChecado1());
                                
                               
 
                                falta.setCrn(crn);
                                falta.setRegistro(checados);
